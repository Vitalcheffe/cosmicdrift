          ...
          <div className="space-y-0">
            {jobs.map((job, i) => (
              <FadeIn key={job.title} delay={i * 0.05}>
                <div
                  className="flex flex-col md:flex-row md:items-center justify-between py-6 border-b border-[rgba(0,0,0,0.04)] gap-4 group"
                >
                  <div>
                    <h3 className="text-base font-bold text-[#0A0F1A] group-hover:text-[#0A0F1A] transition-colors mb-1">
                      {job.title}
                    </h3>
                  ...
                  <button className="inline-flex items-center gap-2 text-xs text-[#9CA3AF] hover:text-[#0A0F1A] border border-[rgba(0,0,0,0.08)] hover:border-[rgba(0,0,0,0.2)] px-5 py-2 rounded-xl transition-colors shrink-0 self-start md:self-center">
                    Apply
                    <ArrowRight size={12} />
                  </button>
                </div>

          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-10 md:gap-16">
            {values.map((value, i) => (
              <FadeIn key={value.title} delay={i * 0.1}>
                <div>
                  <value.icon size={20} className="text-[#C9A84C] mb-4" strokeWidth={1.5} />
                  <h3 className="text-xl font-bold text-[#0A0F1A] mb-3">{value.title}</h3>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-10">
            {leadership.map((person, i) => (
              <FadeIn key={person.name} delay={i * 0.1}>
                <div className="group">
                  <div className="relative w-full aspect-[3/4] mb-5 overflow-hidden rounded-2xl bg-[#F1F3F5]">
                    <Image
                      src={person.image}
                      alt={person.name}
                      fill
                  ...
                  <h3 className="text-base font-bold text-[#0A0F1A] mb-1">{person.name}</h3>
                  <p className="text-xs tracking-[0.05em] text-[#C9A84C] mb-3">{person.title}</p>
        ...
        <div className="max-w-[1400px] mx-auto px-6 md:px-12 text-center">
          <FadeIn>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
              ...
              <Link
                href="/careers"
                className="inline-flex items-center gap-2 border border-[rgba(0,0,0,0.1)] text-[#0A0F1A] px-8 py-4 rounded-xl text-sm font-medium hover:border-[rgba(0,0,0,0.2)] hover:bg-[rgba(0,0,0,0.02)] transition-colors"
              >
                View Careers
              </Link>
            </div>
